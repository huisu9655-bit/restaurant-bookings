...
function migrateSchema() {
  ...
  const hasAuditReportName = columns.some(col => col.name === 'auditReportName');
  if (!hasAuditReportName) {
    db.exec(`ALTER TABLE influencers ADD COLUMN auditReportName TEXT DEFAULT ''`);
  }
  const hasAuditReportText = columns.some(col => col.name === 'auditReportText');
  if (!hasAuditReportText) {
    db.exec(`ALTER TABLE influencers ADD COLUMN auditReportText TEXT DEFAULT ''`);
  }
  const hasCommentDetailName = columns.some(col => col.name === 'commentDetailName');
  if (!hasCommentDetailName) {
    db.exec(`ALTER TABLE influencers ADD COLUMN commentDetailName TEXT DEFAULT ''`);
  }
  const hasCommentDetailText = columns.some(col => col.name === 'commentDetailText');
  if (!hasCommentDetailText) {
    db.exec(`ALTER TABLE influencers ADD COLUMN commentDetailText TEXT DEFAULT ''`);
  }
  ...
  const hasFile = db.prepare('SELECT COUNT(1) as count FROM influencer_files WHERE influencerId = ? AND kind = ?');
  const insertFile = db.prepare(
    'INSERT INTO influencer_files (id, influencerId, kind, fileName, fileText, createdAt) VALUES (?, ?, ?, ?, ?, ?)'
  );
  const clearAudit = db.prepare('UPDATE influencers SET auditReportName = \'\', auditReportText = \'\' WHERE id = ?');
  const clearComment = db.prepare('UPDATE influencers SET commentDetailName = \'\', commentDetailText = \'\' WHERE id = ?');
  for (const inf of influencers) {
    const createdAt = inf.updatedAt || inf.createdAt || now;
    const auditText = normalizeString(inf.auditReportText);
    const commentText = normalizeString(inf.commentDetailText);
    if (auditText) {
      const existingCount = Number(hasFile.get(inf.id, 'audit')?.count || 0);
      if (!existingCount) {
        const fileName = normalizeString(inf.auditReportName) || 'audit.csv';
        insertFile.run(generateId('infFile'), inf.id, 'audit', fileName, auditText, createdAt);
        clearAudit.run(inf.id);
      }
    }
    if (commentText) {
      const existingCount = Number(hasFile.get(inf.id, 'comment')?.count || 0);
      if (!existingCount) {
        const fileName = normalizeString(inf.commentDetailName) || 'comments.csv';
        insertFile.run(generateId('infFile'), inf.id, 'comment', fileName, commentText, createdAt);
        clearComment.run(inf.id);
      }
    }
  }
}
...
function seedFromLegacy() {
  ...
  const insertInfluencer = db.prepare(
    'INSERT INTO influencers (id, displayName, handle, avatarData, contactMethod, contactInfo, notes, profileLink, auditReportName, auditReportText, commentDetailName, commentDetailText, createdAt, updatedAt) VALUES (@id, @displayName, @handle, @avatarData, @contactMethod, @contactInfo, @notes, @profileLink, @auditReportName, @auditReportText, @commentDetailName, @commentDetailText, @createdAt, @updatedAt)'
  );
  ...
  influencers.forEach(inf => {
    ...
    insertInfluencer.run({
      ...
      auditReportName: normalizeString(inf.auditReportName),
      auditReportText: normalizeString(inf.auditReportText),
      commentDetailName: normalizeString(inf.commentDetailName),
      commentDetailText: normalizeString(inf.commentDetailText),
...
function mapInfluencer(row) {
  return {
    ...
    auditReportName: row.auditReportName || '',
    auditReportText: row.auditReportText || '',
    commentDetailName: row.commentDetailName || '',
    commentDetailText: row.commentDetailText || '',
    auditFileCount: Number(row.auditFileCount || 0),
    commentFileCount: Number(row.commentFileCount || 0),
    createdAt: row.createdAt,
    updatedAt: row.updatedAt
  };
}
...
function getAllInfluencers() {
  return db
    ...
    .prepare(
      `SELECT influencers.*,
              (SELECT COUNT(1) FROM influencer_files f WHERE f.influencerId = influencers.id AND f.kind = 'audit') as auditFileCount,
              (SELECT COUNT(1) FROM influencer_files f WHERE f.influencerId = influencers.id AND f.kind = 'comment') as commentFileCount
       FROM influencers
...
function createInfluencer(payload = {}) {
  ...
  db.prepare(
    `INSERT INTO influencers (id, displayName, handle, avatarData, contactMethod, contactInfo, notes, profileLink, auditReportName, auditReportText, commentDetailName, commentDetailText, createdAt, updatedAt)
     VALUES (@id, @displayName, @handle, @avatarData, @contactMethod, @contactInfo, @notes, @profileLink, @auditReportName, @auditReportText, @commentDetailName, @commentDetailText, @createdAt, @updatedAt)`
  ).run(influencer);
  return influencer;
}
  ...
  db.prepare(
    `UPDATE influencers
     SET displayName=@displayName, handle=@handle, avatarData=@avatarData,
         contactMethod=@contactMethod, contactInfo=@contactInfo, notes=@notes, profileLink=@profileLink,
         auditReportName=@auditReportName, auditReportText=@auditReportText,
         commentDetailName=@commentDetailName, commentDetailText=@commentDetailText,
         updatedAt=@updatedAt
     WHERE id=@id`
  ).run(influencer);
...
function deleteInfluencer(id) {
  ...
  db.prepare('DELETE FROM influencer_files WHERE influencerId = ?').run(id);
  db.prepare('DELETE FROM influencers WHERE id = ?').run(id);
  return true;
}

function listInfluencerFiles(influencerId, kind = '') {
  if (!influencerId) throw new Error('未找到达人');
  const kindNorm = normalizeString(kind);
  const rows = kindNorm
    ? db
        .prepare(
          `SELECT id, influencerId, kind, fileName, createdAt
           FROM influencer_files
           WHERE influencerId = ? AND kind = ?
           ORDER BY datetime(createdAt) DESC, rowid DESC`
        )
        .all(influencerId, kindNorm)
    : db
        .prepare(
          `SELECT id, influencerId, kind, fileName, createdAt
           FROM influencer_files
...
function getInfluencerFile(influencerId, fileId) {
  if (!influencerId) throw new Error('未找到达人');
  if (!fileId) throw new Error('未找到文件');
  const row = db
    .prepare(
      `SELECT id, influencerId, kind, fileName, fileText, createdAt
       FROM influencer_files
       WHERE influencerId = ? AND id = ?`
    )
    .get(influencerId, fileId);
...
function createInfluencerFile(influencerId, payload = {}) {
  if (!influencerId) throw new Error('未找到达人');
  const influencer = db.prepare('SELECT id FROM influencers WHERE id = ?').get(influencerId);
  ...
  db.prepare(
    'INSERT INTO influencer_files (id, influencerId, kind, fileName, fileText, createdAt) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(id, influencerId, kind, fileName, fileText, createdAt);
  return { id, influencerId, kind, fileName, createdAt };
}

function deleteInfluencerFile(influencerId, fileId) {
  if (!influencerId) throw new Error('未找到达人');
  if (!fileId) throw new Error('未找到文件');
  const result = db
    .prepare('DELETE FROM influencer_files WHERE influencerId = ? AND id = ?')
    .run(influencerId, fileId);
  if (!result.changes) throw new Error('未找到文件');
  return true;
}
